...
const checkIsTitleInArrey = async (req, res, next) => {
    req.isNew = !Boolean(req.games.find(item => item.title === req.body.title));
    next();
}

 const updateGamesArrey = async  (req, res, next) => {
    if (req.isNew) {
...
module.exports = {getAllGames, checkIsTitleInArrey, updateGamesArrey, updateGamesFile, findGameById, deleteGame}
